...
② 전환     샀다                           (CVR = 주문 / UV)
    ↓
③ 확정     실제로 이용이 확정되었다        (CFR = 확정GMV / GMV)
    ↓
= 공헌이익
```
...
퍼널 각 단계에서 공헌이익에 영향을 주는 부가 지표를 함께 분석합니다:

| 퍼널 단계 | 핵심 분석 | 딥다이브 지표 |
|----------|----------|-------------|
| **① 유입** | 유입 채널별 UV, 광고비 대비 효율(CPA/ROAS) | 채널별 UV 추이, Paid vs Organic 비중 |
| **② 전환** | 체크아웃 퍼널 이탈 (상세→체크아웃→결제 어디서 빠지는가?) | 주문건수, 객단가(AOV), 카테고리별/채널별 CVR 분해 |
| **③ 확정** | 취소 시점/사유 패턴 (언제, 왜 취소하는가?) | 마진율, 쿠폰(CPD) ROI, 파트너별 취소율 |

이 지표들이 **도시별로 어떻게 다르고, 어디가 나빠지고 있는지**를 보여줍니다.
