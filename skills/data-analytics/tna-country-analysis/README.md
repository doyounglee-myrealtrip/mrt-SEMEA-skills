...
```
/tna-country-analysis 스페인      →  비교기준 확인  →  Top 5 도시 자동 식별  →  도시별 분석  →  (선택) 보고서
/tna-country-analysis Barcelona   →  비교기준 확인  →  해당 도시만 분석  →  (선택) 보고서
```

> 분석에 들어가기 전에 **"감소를 어떤 기준으로 볼지"(작년 대비 / 직전월 대비 / 마진율 추세)** 를 먼저 한 번 맞춥니다. 공헌이익이 작년 대비로는 늘었는데 마진율만 빠지는 경우가 흔해서, 한 기준만 보면 원인을 놓치기 때문이에요.
...
| **공헌이익 드라이버 분해** | 공헌이익 변화가 어느 요소(주문수·객단가·확정률·마진율)에서 왔는지 정량 분해 | "1월 공헌이익은 객단가가 끌고, 마진율이 끌어내림" |
...
| **쿠폰 누수 추적 (상품·쿠폰 단위)** | 마진율이 빠질 때 어느 상품/쿠폰이 범인인지 끝까지 추적 | "입장권 쿠폰 누수의 89%가 단일 상품 1개" |
| **객단가 분해 (단가 vs 상품믹스)** | 객단가 변화가 가격 인상 때문인지 고가 상품 비중 증가 때문인지 | "객단가 +25%의 96%가 순수 가격 인상" |
...
기본은 **화면(채팅/터미널)으로 결과를 바로** 보여줍니다. 보고서가 필요하면 원하는 형식으로 받을 수 있어요 — **Confluence 문서 / HTML 시각화 / xlsx / markdown** 중 선택. (자동 발행은 안 하고, 요청할 때만 만듭니다.) 보고서는 비개발자도 읽을 수 있도록 전문용어에 설명을 붙이고, 표/다이어그램/코드블록을 조합하여 작성됩니다.
...
Phase 0: 사전 준비
  ...
  │
  ▼
Phase 0.5: 분석 전 얼라인                          ← 결과를 바꾸는 기준 먼저 확정
  ├── 비교 기준 (작년대비 / 직전월 / 직전피크 / 마진율 추세)
  ├── 분석 기간
  └── 배경/의도 (수익성 방어 / 이상 점검 / 리포트)
  │
  ▼
Phase 1: 현황 진단 (도시 레벨)                    ← 병렬 실행
  ...
  ├── 1-2. 퍼널 6단계 도시별 비교
  ├── 1-3. 전체 성적표
  └── 1-4. 공헌이익 드라이버 분해 (주문×객단가×확정률×마진율)
  │
  ▼
...
Phase 2: 문제별 드릴다운 (카테고리 + 상품 레벨)   ← 문제가 있는 도시만
  ├── 카테고리별 퍼널 지표 분해
  ├── 유입 채널(UTM)별 분해 + 체크아웃 단계 이탈
  ├── 쿠폰 누수 추적 (상품·쿠폰 ID 단위) — 마진율 하락 시
  ├── 객단가 분해 (단가효과 vs 상품믹스효과) — 객단가 급변 시
  └── 취소 TOP 상품 식별 (L3)
  │
  ▼
Phase 3: 심층 분석                                ← 4개 병렬 실행
  ...
  │
  ▼
산출물: 화면 결과 (기본) → 필요 시 보고서 발행 (Confluence/HTML/xlsx/md 중 선택)
```
...
- **기본**: 최근 24개월 (오늘 기준 2년). 특정 기간을 지정하면 그 기간을 우선합니다.
- **비교 방식**: 분석 전에 사용자와 먼저 맞춥니다 — 작년 대비(YoY) / 직전월 대비(MoM) / 직전 피크 대비 / 마진율(CMR) 추세 중 무엇을 볼지. 여러 기준을 함께 봐도 됩니다. (YoY만 단독으로 "감소"를 단정하지 않아요. 작년 대비로는 늘었는데 마진율만 빠지는 경우가 많거든요.) YoY를 쓸 때는 시즌 효과 제거를 위해 같은 시기끼리 비교합니다.
...
### Q: 보고서는 어떻게 받나요?
기본은 화면(채팅/터미널)으로 바로 보여줍니다. 보고서가 필요하면 요청하세요 — Confluence 문서(개인 스페이스에 발행) / HTML 시각화 / xlsx / markdown 중 선택할 수 있어요. 자동 발행은 하지 않습니다.

### Q: "감소"라고 했는데 왜 비교 기준을 먼저 물어보나요?
공헌이익은 작년 대비로는 늘었는데 마진율만 빠지는 경우가 흔합니다. 어떤 기준(작년 대비 / 직전월 / 마진율 추세)으로 보느냐에 따라 결론이 정반대가 될 수 있어서, 분석 전에 한 번 맞춥니다. 헷갈리면 "알아서 해줘"라고 하시면 가장 안전한 기준으로 진행하고 어떤 기준을 썼는지 알려드려요.
