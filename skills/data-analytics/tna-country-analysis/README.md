...
| 퍼널 단계 | 부가 분석 지표 | 무엇을 알 수 있나 |
|----------|-------------|------------------|
| **① 유입** | 유입 채널별 UV (UTM source) | 어떤 채널에서 사람이 오는가, 광고비 대비 효율은? |
| **② 전환** | 주문건수, 객단가(AOV), 체크아웃 퍼널 이탈 | 몇 건을 샀고, 건당 얼마이고, 어느 단계에서 빠지는가? |
| **③ 확정** | 취소 시점/사유 패턴, 마진율, 쿠폰(CPD) ROI | 왜 취소하고, 확정 건에서 얼마가 남는가? |
