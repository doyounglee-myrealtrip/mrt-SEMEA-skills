...
| 퍼널 단계 | 부가 지표 | 정의 | 용도 |
|----------|----------|------|------|
| ① 유입 | 유입 채널별 UV (UTM) | 채널별 유입량 + 광고비/ROAS | 마케팅 예산 효율 진단 |
| ② 전환 | 주문건수 (Orders) | 절대 주문 볼륨 | 수요 규모 파악 |
| ② 전환 | 객단가 (AOV = GMV / Orders) | 건당 평균 거래액 | 가격/믹스 변화 파악 |
| ② 전환 | 체크아웃 퍼널 이탈 | 상세→체크아웃→결제 단계별 전환율 | CVR 하락 원인 특정 |
| ③ 확정 | 취소 시점/사유 패턴 | 예약→취소 간격, 이용일 대비 취소 타이밍 | 취소 유형 분류 |
| ③ 확정 | 마진율 (CON_MARGIN / CGMV) | 비용 차감 후 남는 비율 | 수익성 진단 |
| ③ 확정 | 쿠폰(CPD) ROI | CPD 투입 대비 증분 GMV/CM | 프로모션 효율 진단 |
